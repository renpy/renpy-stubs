...
def wrap_hide(tree: ast.AST) -> None: ...
...
def construct_None(*args: Any) -> None: ...

type CompileMode = Literal["eval", "exec", "hide"]

class CompileCache:
    BYTECODE_VERSION: int
    OLD_BYTECODE_FILE: Final[str]
    BYTECODE_FILE: Final[str]
    MAGIC_NUMBER: Final[str]
    type ItemKey = tuple[int, int, str, CompileMode, int, int]
    type CacheKey = tuple[int, int, str, CompileMode, bytes, int, int]
    type WarningsKey = tuple[Literal["warnings"], CacheKey]
    type LiteralValue = tuple[Literal["literal"], Any]
    old_compile_cache: dict[CompileCache.CacheKey, types.CodeType | CompileCache.LiteralValue]
    new_compile_cache: dict[CompileCache.CacheKey, types.CodeType | CompileCache.LiteralValue]
    old_bytecode_cache: dict[CompileCache.CacheKey, bytes]
    new_bytecode_cache: dict[CompileCache.CacheKey, bytes]
    warnings: dict[CompileCache.WarningsKey, list[tuple[str, int, str]]]
    bytecode_dirty: bool
    def __init__(self) -> None: ...
    def load(self) -> None: ...
    def save(self) -> None: ...
    ...
    def put(self, item_key: ItemKey, value: types.CodeType | LiteralValue, warnings: list[tuple[str, int, str]]): ...
